...
def main():
    ...
    if request.method == "GET":
        ...
        form = request.form
        if not ("cultureid" in form and "culturepw" in form):
            abort(400)
        con, cur = fc.start_db(session["id"])
        cur.execute("UPDATE configs SET cid = ?, cpw = ?;",
                    (form["cultureid"], form["culturepw"]))
        con.commit()
        con.close()
        return "ok"
...
def product(productid):
    ...
    if request.method == "GET":
        ...
        con, cur = fc.start_db(session["id"])
        cur.execute("UPDATE products SET name = ?, price = ?, stocks = ? WHERE id == ?;",
                    (form["name"], form["price"], form["stocks"], productid))
        con.commit()
        con.close()
        return "ok"
    else:
...
def license():
    if not "id" in session:
        return redirect("/login")
    if request.method == "GET":
        license_remaining = licensing.get_remaining_string(fc.guild_info(session["id"])[
                                                           0]) if fc.is_guild_valid(session["id"])[1] == True else "0일 0시간 0분 [ 만료 ]"
        ...
        new_license_remaining = licensing.add_time(guild_info[0], key_info[1]) if licensing.is_expired(
            guild_info[0]) == False else licensing.make_new_expiringdate(key_info[1])
        con, cur = fc.start_db(session["id"])
